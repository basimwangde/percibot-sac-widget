/* PerciBot — SAC Chat Widget
   Sends user queries to the PerciBOT FastAPI backend.
...
;(function () {
  ...
  class PerciBot extends HTMLElement {
    constructor () {
      ...
      // ── Default props — includes all new backend/prompt fields ──────────────
      this._props = {
        // Existing
        apiKey:       '',
        model:        'gpt-3.5-turbo',
      ...
      // Stable per-session identifier. Generated once per widget instance
      // and reused for all subsequent requests so the backend can correlate
    ...
    onCustomWidgetAfterUpdate (changedProps = {}) {
      ...
      // Update hint based on backend URL availability
      ...
      // Parse pushed datasets if present
    ...
    /**
     * Sends the user query to the PerciBOT FastAPI backend (/ask endpoint).
     *
     * Request body contract:
     *   query            — user's natural-language question
     *   session_id       — stable per-widget-instance UUID for conversation memory
     *   answer_prompt    — L2 answer generation template (may be large)
     *   behaviour_prompt — L1 SQL behaviour rules (may be large)
     *   schema_prompt    — active table schema description (may be large)
     *   client_id        — demo/client context identifier
     *
     * Response contract:
     *   data.answer   — rendered to user when present (COMPLETED state)
     *   data.message  — rendered to user when answer is absent (TERMINATED/FAILED state)
     */
    async _send () {
      ...
      if (!backendUrl) {
        this._append(
          'bot',
          '\u26a0\ufe0f Backend URL not configured. Open the Builder panel to set it.'
        )
        return
      }
      ...
      try {
        ...
        // Resolve display text: prefer answer, fall back to message
        const displayText = (data.answer && data.answer.trim())
          ? data.answer
          : (data.message || '(No response received from backend)')

        this._stopTyping()
        this._append('bot', displayText)
    ...
    // ── Dataset context builder (kept for potential future use) ────────────────
    ...
    _buildDatasetContext (opts = {}) {
      ...
      const lines = ['You have access to the following datasets:']
